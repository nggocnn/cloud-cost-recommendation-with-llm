            ...
            for key, preferred_col in column_mapping.items():
                if preferred_col in df.columns:
                    ...
                    for col in df.columns:
                        if key.lower() in col.lower() or preferred_col.split('/')[-1].lower() in col.lower():
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    billing_record = BillingData(
                        account_id=str(row.get(actual_columns.get('account_id', ''), '')),
                        service=str(row.get(actual_columns.get('service', ''), '')),
                        resource_id=str(row.get(actual_columns.get('resource_id', ''), '')) if pd.notna(row.get(actual_columns.get('resource_id', ''))) else None,
                        region=str(row.get(actual_columns.get('region', ''), '')),
                        usage_type=str(row.get(actual_columns.get('usage_type', ''), '')),
                        usage_amount=float(row.get(actual_columns.get('usage_amount', ''), 0)),
                        usage_unit=str(row.get(actual_columns.get('usage_unit', ''), '')),
                        unblended_cost=float(row.get(actual_columns.get('unblended_cost', ''), 0)),
                        amortized_cost=float(row.get(actual_columns.get('amortized_cost', ''), 0)),
                        bill_period_start=pd.to_datetime(row.get(actual_columns.get('bill_period_start', ''))),
                        bill_period_end=pd.to_datetime(row.get(actual_columns.get('bill_period_end', ''))),
                        tags=tags
                    )
                    ...
                    logger.warning("Failed to parse billing record", row_index=_, error=str(e))
                    continue
            ...
            for item in inventory_data:
                try:
                    ...
                    resource = Resource(
                        ...
                        created_at=pd.to_datetime(item.get('created_at')) if item.get('created_at') else None,
                    ...
                    logger.warning("Failed to parse inventory record", item=item, error=str(e))
                    continue
        ...
        try:
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    metrics_dict = {}
                    for col in df.columns:
                        if col not in ['resource_id', 'timestamp', 'period_days'] and pd.notna(row[col]):
                            try:
                    ...
                    logger.warning("Failed to parse metrics record", row_index=_, error=str(e))
                    continue

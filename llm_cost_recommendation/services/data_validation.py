...
class DataQualityValidator:
    ...
    def validate_resource_data(self, 
                             ...
                             billing_data: Optional[List[BillingData]] = None) -> Dict[str, Any]:
        ...
        validation_result["recommendations_possible"] = (
            validation_result["data_availability"]["inventory"] and
            (validation_result["data_availability"]["billing"] or 
             validation_result["data_availability"]["metrics"])
        )
    ...
    def _validate_inventory_data(self, resource: Resource, validation_result: Dict) -> float:
    ...
    def _validate_billing_data(self, billing_data: Optional[List[BillingData]], validation_result: Dict) -> float:
    ...
    def _validate_metrics_data(self, metrics: Optional[Metrics], validation_result: Dict) -> float:
        ...
        if metrics and metrics.resource_id:
            ...
            if any(getattr(metrics, attr, None) is not None 
                   for attr in ['cpu_utilization_p50', 'cpu_utilization_p90', 'cpu_utilization_p95']):
                score += 0.3
            else:
                validation_result["data_quality_issues"].append(
            ...
            if any(getattr(metrics, attr, None) is not None
                   for attr in ['memory_utilization_p50', 'memory_utilization_p90', 'memory_utilization_p95']):
                score += 0.2
            else:
                validation_result["data_quality_issues"].append(
    ...
    def get_data_quality_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "common_issues": [f"{issue} ({count} resources)" for issue, count in common_issues],
            "recommendations": [f"{rec} ({count} resources)" for rec, count in common_recs],
            "average_quality_score": sum(r["quality_score"] for r in self.validation_results.values()) / total
        }
...
class EvidenceValidator:
    ...
    def validate_recommendation_evidence(self, 
        ...
        self._validate_confidence_alignment(confidence_score, evidence, available_data, validation)
        
        # Calculate evidence score
        validation["evidence_score"] = self._calculate_evidence_score(validation, available_data)
    ...
    def _validate_metrics_claims(self, metrics_claim: str, available_data: Dict, validation: Dict):
        ...
        percentage_indicators = [
            "% of requests", "% of traffic", "% utilization", "traffic shows",
            "analysis shows", "observed traffic", "requests are served",
            "traffic analysis over", "performance data indicates"
        ]
        
        has_percentage_claim = any(indicator in metrics_claim.lower() for indicator in percentage_indicators)
        has_specific_percentage = any(char.isdigit() and "%" in metrics_claim[i:i+5] 
                                    for i, char in enumerate(metrics_claim) if char.isdigit())
        
        if has_percentage_claim or has_specific_percentage:
            if not available_data.get("metrics") or not available_data.get("metrics", {}).get("datapoints"):
                validation["unsupported_claims"].append(
                    f"Traffic/Performance claim: '{metrics_claim[:100]}...' - but no performance data available"
                )
                validation["evidence_issues"].append("Fabricated traffic percentage without actual metrics")
                
    def _validate_cost_claims(self, cost_claim: str, available_data: Dict, validation: Dict):
        """Validate cost-related claims in evidence"""
        if not cost_claim:
            return
            
        # Check for specific cost numbers
        if "$" in cost_claim and ("current" in cost_claim.lower() or "monthly" in cost_claim.lower()):
            if not available_data.get("billing_data"):
                if "estimated" not in cost_claim.lower() and "approximate" not in cost_claim.lower():
                    validation["evidence_issues"].append("Cost claim doesn't indicate if data is estimated vs actual")
        
        # Check for contradiction: saying "no cost data" but providing specific amounts
        no_cost_data_indicators = [
            "no actual cost data", "no cost data available", "cost data not available",
            "no billing data", "billing data not available"
        ]
        
        if any(indicator in cost_claim.lower() for indicator in no_cost_data_indicators):
            validation["unsupported_claims"].append(
                f"Cost contradiction: Evidence claims '{cost_claim}' but specific dollar amounts provided in recommendation"
            )
            validation["evidence_issues"].append("Fabricated cost estimates without actual billing data")
                    
    def _validate_performance_claims(self, performance_claim: str, available_data: Dict, validation: Dict):
        ...
        performance_indicators = [
            ...
            "performance remains", "expected performance", "impact"
        ]
        
        has_performance_claim = any(indicator in performance_claim.lower() for indicator in performance_indicators)
    ...
    def _validate_confidence_alignment(self, confidence_score: float, evidence: Dict, available_data: Dict, validation: Dict):
        ...
        no_data_indicators = [
            "no performance metrics", "no actual cost data", "no cost data available",
            "metrics not available", "data not available", "insufficient data"
        ]
        
        has_data_limitations = any(indicator in evidence_statements for indicator in no_data_indicators)
        
        # Flag high confidence with poor data availability
        if confidence_score > 0.6 and (data_availability_score < 0.4 or has_data_limitations):

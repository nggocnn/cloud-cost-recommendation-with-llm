...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Get logger instance (in case logging was reconfigured)
...
async def lifespan(app: FastAPI):
    ...
    # Ensure logging is configured for the API
    log_level = app_state.get("log_level", "INFO")
    log_format = app_state.get("log_format", "auto")
    configure_logging(level=log_level, format_type=log_format, component="api")

    # Get fresh logger instance after configuration
    api_logger = get_logger(__name__)

    # Startup
    api_logger.info("Starting LLM Cost Recommendation API")
    ...
    try:
        ...
        api_logger.info(
            ...
            enabled_agents=len(
                [
                    a
                    for a in coordinator.service_agents.values()
        ...
        api_logger.error("Failed to initialize application", error=str(e))
        raise

    yield

    # Shutdown
    api_logger.info("Shutting down LLM Cost Recommendation API")
...
async def value_error_handler(request: Request, exc: ValueError):
    ...
    error_response = APIError(
        error=str(exc), code="VALIDATION_ERROR", details={"type": "ValueError"}
    )
    ...
    try:
        ...
        if request.individual_processing:
            ...
            for resource in request.resources:
                ...
                resource_metrics = [
                    m for m in request.metrics if m.resource_id == resource.resource_id
                ]
                resource_billing = [
                    b for b in request.billing if b.resource_id == resource.resource_id
                ]
            ...
            report = await coordinator.analyze_resources_and_generate_report(
                request.resources, metrics_data, billing_data
            )
        ...
        if request.individual_processing:
            ...
            total_monthly_savings = sum(
                r.estimated_monthly_savings for r in final_recommendations
            )
            total_annual_savings = sum(r.annual_savings for r in final_recommendations)

            # Count risk levels
            low_risk_count = len(
                [r for r in final_recommendations if r.risk_level == RiskLevel.LOW]
            )
            medium_risk_count = len(
                [r for r in final_recommendations if r.risk_level == RiskLevel.MEDIUM]
            )
            high_risk_count = len(
                [r for r in final_recommendations if r.risk_level == RiskLevel.HIGH]
            )
...
async def get_recommendation_detail(recommendation_id: str):
    ...
    raise HTTPException(
        status_code=501, detail="Recommendation detail endpoint not implemented"
    )
...
def run_server(
    ...
    # Store config in app state (including logging config for lifespan manager)
    app_state.update(
        {
            "config_dir": config_dir,
            "data_dir": data_dir,
    ...
    try:
        # Configure uvicorn with proper settings for continuous operation
        uvicorn_config = {
            "app": "llm_cost_recommendation.api:app",
            "host": host,
            "port": port,
        ...
        # Add reload for development or workers for production
        if reload:
            uvicorn_config["reload"] = True
            uvicorn_config["reload_dirs"] = ["llm_cost_recommendation"]
            server_logger.info("Development mode enabled with auto-reload")
        else:
            # Use single worker to avoid app state sharing issues
            # Multiple workers would require shared state management
            uvicorn_config["workers"] = 1
            server_logger.info("Production mode enabled")

        server_logger.info("Server configuration ready", config=uvicorn_config)

        # Run server - this blocks until the server is stopped
        uvicorn.run(**uvicorn_config)

    except Exception as e:
        server_logger.error("Failed to start server", error=str(e))
        raise
    finally:

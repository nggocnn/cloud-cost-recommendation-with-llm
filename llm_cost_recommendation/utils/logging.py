...
def colorize_service(service: str) -> str:
    ...
    return service_colors.get(
        service, f"{ColorFormatter.CYAN}{service}{ColorFormatter.RESET}"
    )
            ...
            if isinstance(value, (int, float)):
                if key.endswith("_count") or key.endswith("_savings") or key == "count":
                    context_fields.append(
                        f"{ColorFormatter.GREEN}{key}={value}{ColorFormatter.RESET}"
                    )
                else:
                    context_fields.append(f"{key}={value}")
            elif isinstance(value, str) and len(value) < 50:
                context_fields.append(
                    f"{key}={ColorFormatter.CYAN}{value}{ColorFormatter.RESET}"
                )

...
from .types import CloudProvider, ServiceTypeUnion, RecommendationType, RiskLevel
...
class AnalysisRequest(BaseModel):
    ...
    resources: List[Resource] = Field(min_length=1, description="List of cloud resources to analyze")
    billing: List[BillingData] = Field(default_factory=list, description="Billing data for resources")
    metrics: List[Metrics] = Field(default_factory=list, description="Performance metrics for resources")
...
class FilterParams(BaseModel):
    ...
    def max_greater_than_min(cls, v, values):
        if v is not None and 'min_savings' in values.data and values.data['min_savings'] is not None:

...
from ..models import (
    Resource, Metrics, BillingData, Recommendation, RecommendationReport,
    ServiceType, CoordinatorConfig, ServiceAgentConfig, RiskLevel
)
...
class ServiceAgentFactory:
    ...
    def create_agent(config: ServiceAgentConfig, llm_service: LLMService) -> BaseAgent:
        ...
        # For now, we'll use a generic LLM-based agent for most services
        # and rule-based agent for simple cases
        
        if config.service in [ServiceType.ELASTIC_IP, ServiceType.EBS]:
            return RuleBasedAgent(config, llm_service)
        else:
...
class LLMServiceAgent(BaseAgent):
    ...
    async def analyze_resource(
        ...
        try:
            ...
            llm_recommendations = await self._generate_recommendations_from_llm(context_data)
            ...
            logger.error("Failed to analyze resource with LLM", 
                        agent_id=self.agent_id,
                        resource_id=resource.resource_id,
                        error=str(e))
...
class CoordinatorAgent:
    ...
    def _initialize_agents(self):
        ...
        for service in self.config.enabled_services:
            ...
            if service_config and service_config.enabled:
                try:
                    agent = ServiceAgentFactory.create_agent(service_config, self.llm_service)
                    ...
                    logger.error("Failed to initialize service agent", 
                                service=service.value,
                                error=str(e))
    ...
    async def analyze_account(
        ...
        if analysis_tasks:
            service_recommendations = await asyncio.gather(*analysis_tasks, return_exceptions=True)
        ...
        processed_recommendations = self._post_process_recommendations(all_recommendations)
    ...
    def _group_resources_by_service(self, resources: List[Resource]) -> Dict[ServiceType, List[Resource]]:
    ...
    async def _analyze_service_resources(
        ...
        try:
            ...
            if len(recommendations) > max_recommendations:
                ...
                logger.info("Applied recommendation limit", 
            ...
            logger.error("Failed to analyze service resources", 
                        service=agent.service.value,
                        error=str(e))
            return []
    
    def _post_process_recommendations(self, recommendations: List[Recommendation]) -> List[Recommendation]:
    ...
    def _deduplicate_recommendations(self, recommendations: List[Recommendation]) -> List[Recommendation]:
        ...
        for resource_id, resource_recs in by_resource.items():
            if len(resource_recs) == 1:
                ...
                logger.debug("Deduplicated recommendations", 
    ...
    def _rank_recommendations(self, recommendations: List[Recommendation]) -> List[Recommendation]:
        ...
        def calculate_score(rec: Recommendation) -> float:
            # Normalize factors
            savings_score = min(rec.estimated_monthly_savings / 100.0, 1.0)  # Normalize to 0-1
    ...
    def _generate_report(
        ...
        for rec in recommendations:
            ...
            elif rec.risk_level == RiskLevel.MEDIUM or len(rec.implementation_steps) <= 5:
    ...
    async def analyze_resource(
        ...
        if resource.service not in self.service_agents:
            logger.warning("No agent available for service", 
